...
def output_data(allPrices, n_steps, n_simulations):
    ...
    for i in range(len(x_axis)):
        ...
        #Store probability of final price being 1000
        if x_axis[i] == 1000:
            probOfPrice1000 = y_axis[i]*100

    #plt.show()
    plt.savefig('../outputs/ProbDistInt_S='+str(n_steps)+'_N='+str(n_simulations),bbox_inches='tight')
    ...
    with open('../outputs/ProbInt_Price=1000_S='+str(n_steps)+'_N='+str(n_simulations)+'.txt', 'w') as myFile:
...
def monte_carlo_Simulations(n_steps, n_simulations):
    ...
    #Generate an NxS matrix of random numbers, either -1 or 1 with equal probability.
    ...
    randomNumberMatrix = np.random.choice([-1, 1], size=(n_simulations, n_steps))

    #Calculate final price for each simulation
    allPrices={}
    for simulationRandomNums in randomNumberMatrix:

        #numpy.sum is the fastest summation in python
        price = 1000 + np.sum(simulationRandomNums)
...
if __name__ == '__main__':
    ...
    '''Example usage:
        In terminal:
        python randomWalkInteger.py 100 10000
    ...
    try:
        ...
        print("python randomWalkInteger.py 50 1000 \n")
